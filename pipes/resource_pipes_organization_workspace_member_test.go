		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckOrganizationWorkspaceMemberExists("pipes_organization_workspace_member.test"),
					resource.TestCheckResourceAttr(
						"pipes_organization_workspace_member.test", "role", "reader"),
				),
			},
			{
				Config: testAccOrganizationWorkspaceMemberUpdateConfig(orgHandle, workspaceHandle),
				Check: resource.TestCheckResourceAttr(
					"pipes_organization_workspace_member.test", "role", "owner"),
			},
		},
	})
}
...
resource "pipes_organization_member" "test" {
	organization = pipes_organization.test.handle
...
resource "pipes_organization_workspace_member" "test" {
	organization = pipes_organization.test.handle
	workspace_handle = pipes_workspace.test.handle
	user_handle      = pipes_organization_member.test.user_handle
	role         = "reader"
...
func testAccCheckOrganizationWorkspaceMemberDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type == "pipes_organization_workspace_member" {
			// Extract organization handle and user handle from ID
			id := rs.Primary.ID
			idParts := strings.Split(id, "/")
			if len(idParts) < 3 {
