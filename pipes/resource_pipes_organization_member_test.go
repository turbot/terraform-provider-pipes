		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckOrganizationMemberExists("pipes_organization_member.test"),
					resource.TestCheckResourceAttr(
						"pipes_organization_member.test", "role", "member"),
				),
			},
			{
				Config: testAccOrganizationMemberUpdateConfig(orgHandle),
				Check: resource.TestCheckResourceAttr(
					"pipes_organization_member.test", "role", "owner"),
			},
		},
	})
}
...
func testAccCheckOrganizationMemberDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type == "pipes_organization_member" {
			// Extract organization handle and user handle from ID
			id := rs.Primary.ID
			idParts := strings.Split(id, "/")
			if len(idParts) < 2 {
