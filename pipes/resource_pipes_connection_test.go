...
func TestAccConnection_Basic(t *testing.T) {
	resourceName := "pipes_connection.test"
...
func testAccCheckConnectionDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if isUser {
			...
			if err != nil {
				return fmt.Errorf("inside testAccCheckConnectionDestroy. getUserHandler Error: \n%v", r)
			}
...
func testCheckJSONString(name, key, value string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if v, ok := rs.Primary.Attributes[key]; !ok {
			configString, _ := formatConnectionJSONString(v)
			compareValue, _ := formatConnectionJSONString(value)
