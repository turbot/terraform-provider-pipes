...
page_title: "pipes_workspace_flowpipe_mod_variable Resource - terraform-provider-pipes"
subcategory: ""
description: |-
  The `Turbot Pipes Workspace Flowpipe Mod Variable` represents variables for a flowpipe mod installed in the workspace.
---

# Resource: pipes_workspace_flowpipe_mod_variable

Manages a workspace flowpipe mod variable.

## Example Usage

**Set a user workspace flowpipe mod variable**
...
resource "pipes_workspace_flowpipe_mod" "aws_thrifty" {
  ...
  path = "github.com/turbot/steampipe-mod-aws-thrifty"
}

resource "pipes_workspace_flowpipe_mod_variable" "mandatory_tags" {
  workspace_handle = pipes_workspace.dev_workspace.handle
  mod_alias = pipes_workspace_flowpipe_mod.aws_thrifty.alias
  name = "notification_level"
  setting_value = jsonencode("error")
}
```

**Set an organization workspace flowpipe mod variable**
